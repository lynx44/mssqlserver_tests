...
describe 'mssqlserver_tests::database_provider_drop' do
  let(:chef_run) do
  ...
  it 'passes expected values' do
    expect(converge).to drop_mssqlserver_database(node['description']).with({
  ...
  it 'executes delete script' do
    sql_command = "IF EXISTS(SELECT Name FROM sys.databases WHERE Name='#{node['database']}') BEGIN DROP DATABASE [#{node['database']}] END"
    expect(converge).to run_mssqlserver_sql_command("drop #{node['database']} database").with({
        :username => node['username'],
        :password => node['password'],
        :database => 'master',
        :instance => node['instance'],
